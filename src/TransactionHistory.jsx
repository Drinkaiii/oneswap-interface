...
const TransactionHistory = ({ latestTransaction }) => {
    const { web3, account } = useContext(WalletContext);
    const [transactions, setTransactions] = useState([]);

    // make the newsest transaction data on the top
    useEffect(() => {
    ...
    const columns = [
      ...
      },
      {
        ...
        key: 'tokenIn',
        render: (symbol) => (symbol ? symbol : 'UNKNOWN'),
      },
      {
        title: 'Sell Amount',
        dataIndex: 'amountIn',
    ...
    return (
      <div>
        ...
        <Table dataSource={transactions} columns={columns} rowKey="transactionHash" />
      </div>
    );
  };
